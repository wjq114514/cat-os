...
int32_t syscall_dispatch(uint32_t nr,uint32_t n,const uint32_t *a){
    ...
    case CATOS_SYS_LISTEN:{socket_t *s=sock_fd((int)a[0]);if(!s)return sock_err((int)a[0]);return s->type==SOCK_TCP_LISTEN?0:-CATOS_EINVAL;}
    case CATOS_SYS_ACCEPT:{socket_t *s=sock_fd((int)a[0]);if(!s)return sock_err((int)a[0]);socket_t *nxt=tcp_accept_socket(s);if(!nxt)return -CATOS_EAGAIN;int fd=vfs_socket_install(nxt);if(fd<0){net_socket_close(nxt);return fd;}return fd;}
    ...
    case CATOS_SYS_SEND:{socket_t *s=sock_fd((int)a[0]);if(!s)return sock_err((int)a[0]);if(bad_user((void*)a[1],a[2],0))return -CATOS_EFAULT;return tcp_send(s,(const uint8_t*)a[1],a[2]);}
    case CATOS_SYS_RECV:{socket_t *s=sock_fd((int)a[0]);if(!s)return sock_err((int)a[0]);if(bad_user((void*)a[1],a[2],1))return -CATOS_EFAULT;int r=tcp_recv(s,(uint8_t*)a[1],a[2]);return r<0?-CATOS_EAGAIN:r;}
