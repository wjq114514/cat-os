...
int user_access_ok(uintptr_t v,size_t n,uint32_t w){if(v<0x1000u||n>0xBFC00000u-v)return 0;uintptr_t e=v+n;for(uintptr_t p=v;p<e;p=(p&~(PAGE_SIZE-1u))+PAGE_SIZE){pde_t d=kernel_page_directory.entries[PDE_INDEX(p)];if(!(d&_PAGE_PRESENT)||!(d&_PAGE_USER))return 0;if(d&_PAGE_PSE)continue;page_table_t *t=(page_table_t*)phys_to_virt(d&PAGE_FRAME_MASK);pte_t x=t->entries[PTE_INDEX(p)];if(!(x&_PAGE_PRESENT)||!(x&_PAGE_USER)|| (w&&!(x&_PAGE_RW)))return 0;}return 1;}
