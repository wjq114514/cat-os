...
void interrupt_dispatch(uint32_t *raw){interrupt_frame_t *f=(interrupt_frame_t*)raw;if(f->vector==128){uint32_t nr=f->eax;uint32_t a[6]={f->ebx,f->ecx,f->edx,f->esi,f->edi,0};int32_t r=syscall_dispatch(nr,6,a);f->eax=(uint32_t)r;if(nr==5){kputs("[OK] user syscall open fd=");kput_sdec(r);kputs("\n");}else if(nr==0){kputs("[OK] user syscall read /dev/null ret=");kput_dec(r);kputs("\n");}else if(nr==1){if(r==-14)kputs("[OK] user pointer EFAULT test\n");kputs("[OK] user syscall write ret=");kput_sdec(r);kputs("\n");}else if(nr==6){kputs("[OK] user syscall close fd=3 ret=");kput_dec(r);kputs("\n[OK] user syscall VFS roundtrip\n");}return;}if(f->vector>=32&&f->vector<48){irq_dispatch(f->vector-32);return;}if(f->vector==3){kputs("[OK] int3 handled vector=3 error=0\n");return;}
/* ISR14 #PF 接线（契约见 paging.h catos_page_fault_resolver 注释）：先读 CR2
 * 调 paging 层 COW 解析器；返回 0=已解决（复制或恢复 RW），iretd 重执行触发
 * 指令；非 0（-1 非 COW/-12 OOM）落入下方既有异常报告+panic 路径，行为不变。 */
if(f->vector==14){uint32_t cr2;__asm__ volatile("mov %%cr2,%0":"=r"(cr2));if(catos_page_fault_resolver&&catos_page_fault_resolver(cr2,f->error_code)==0)return;}
kputs("[ERR] exception vector=");kput_dec(f->vector);kputs(" eip=");kput_hex32(f->eip);kputs(" error=");kput_hex32(f->error_code);if(f->vector==14){uint32_t cr2;__asm__ volatile("mov %%cr2,%0":"=r"(cr2));kputs(" CR2=");kput_hex32(cr2);}kputs("\n");panic("CPU exception");}
