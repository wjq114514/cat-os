# Cat-OS NGINX 移植实施设计（NGINX_PORT_PLAN，含当前实证回填）

> **文档性质**：本文主体是 2026-08-25 的实施设计快照；§0 为 2026-08-28 当前状态回填，优先于历史路线中的“待实施”描述。
> **历史基线**：初版基于 `HEAD=\`6796bd6\`` + 工作区未提交变更，行号和并行锁说明只适用于当时快照。
> **当前结果**：nginx 1.26.2 单进程静态 HTTP 已落地，并经 r4 与独立 fresh QEMU 复验；实现细节见 `NOTES_NGINX_PORT.md`。
> **历史表格说明**：§1、§4 等表中“已修改·未提交”只描述 2026-08-25 设计快照；当前源码提交状态以实现基线 `be876b6` 和 `git status` 为准。
> **状态标记**：✅实测 = 本文作者已读码证实；🔗引用 = 转引自前置文档（未重复核实）；🔍待验证 = 合理推断未运行复现。

## 0. 当前实施结果（2026-08-28）

原计划的 Step 1→4 已被实际实现路径压缩为一个可验证的最小闭环：

| 计划能力 | 当前结果 | 证据 |
|---|---|---|
| Step 2 文件来源 | IDE 磁盘上的 FAT16 只读挂载，VFS 可读取 nginx 配置和静态页 | `fs/fat16.c`、`fs/vfs.c`；r4 `[FS] FAT16 mounted: CATOS` |
| Step 3 用户态服务 | shell 通过 `/bin/nginx` 启动嵌入式 nginx ELF，nginx harness 固定配置路径 | `shell_user.c`、`kernel/syscall.c`、`nginx_harness.c`；r4 start PASS |
| Step 4 事件循环 | `poll` 模块 + nr=168 poll syscall，`writev`/accept/时间等最小 shim 已接线 | `nginx.conf`、`kernel/syscall.c`、`libc/src/posix_stubs.c`；r4 HTTP PASS |
| HTTP 验收 | `/` 200 且正文包含 `Cat-OS nginx works`；缺失路径 404 | r4 + 独立复验结果和串口；两次均 QEMU rc=0 |
| shell 扩展 | `nginx`、`netstat`、`ping <IPv4>` 已可用；重复 `nginx` 有保护 | `/tmp/catos-nginx-rebuild-20260828r4.result`、`/tmp/catos-nginx-doc-verify.result` 及对应 `.serial` |

本轮交付的 nginx 形态是 `master_process off` 的单进程静态服务器。原计划中的完整
master/worker、热升级、完整 signal、upstream/connect、可写日志、epoll 和高并发
扩容仍是后续里程碑，不能由当前 PASS 推断已完成。

---

## 目录

0. [当前实施结果](#0-当前实施结果2026-08-28)
...
6. [历史 NOT_TESTED 与当前验证边界](#6-历史-not_tested-与当前验证边界)
...
| 22 | listen(fd,backlog) | ⚠️有缺口 | 未 bind 先 listen → `-EINVAL`（历史 L1 缺口）；当前 backlog 上限为 `TCP_MAX_CONNS=64`（`net/net.h:86-101`） |
...
**TCP 栈能力（历史行号快照）**：被动开放全链路；SACK≤2 块/OOO 2 槽/scoreboard 8 段/NewReno 风格拥塞控制；动态 RTO + persist 探针；优雅关闭 FIN_WAIT/LAST_ACK/TIME_WAIT 200 ticks。主动开放 connect 仍不存在；当前实现细节以 `net/net_tcp.c`、`net/net.h` 和 §0 为准。
...
**历史加固记录（code4，后续已进入主线）**：H2 修复、M1 R2 重传放弃、M3 盲 RST 防御、TIME-WAIT 化身替换和 scoreboard 满合并降级均已在后续提交/回归资料中收敛；本段行号只用于追溯当时快照。
...
**历史目标**：httpd 迁出内核地址空间，成为经 nr=11 exec 拉起的 ring3 进程，走完整 int 0x80 socket 组；该目标已有 M0 验证。nginx 用户态进程形态已在 §0 以嵌入式 ELF + shell `nginx` 命令落地，但仍明确裁剪为单进程。
...
- **涉及文件（全部热区，§5 串行）**：
  ...
  - `syscall.h/syscall.c`：当前提供 nr=168 `poll(fds*, nfds, timeout_ms)`；nginx r4 已使用，完整阻塞事件队列仍留待后续；
...
> 编号体系出处：`docs/SOCKET_API.md` §6（转引 code8 登记）。本节的「已修」表述属于历史快照；当前主线状态以 §0 和 `docs/TEST_MATRIX.md` 为准。
...
| **H2** | TCP bind 同端口"附着"返 0 | 双 httpd 实例静默共享同一 TCB → 任一方 close 作废另一方 conn 指针（use-after-free 形态）；accept 归属混乱 | **已进入主线**：冲突返 `-EADDRINUSE`（`net/net.c`） | 已由网络回归覆盖；Step 3 端口规划仍规避冲突 |
| **M1** | RTO R1/R2 度量与放弃 | 无 R2 阈值时黑洞对端无限重传占死 64 连接表 → 整站拒绝服务 | **已进入主线**：SYN 3 次/数据 5 次放弃释放（`net/net_tcp.c`） | 网络回归和注入套件覆盖；继续观察高并发容量 |
| **M2** | 错误码混叠（裸 -1 哨兵） | 当前发送缓冲满返回 `-EAGAIN`；其它底层 `-1` 仍可能由 `sock_xlate` 折叠为 EAGAIN，错误原因仍不完全可区分 | **部分收敛** | 应用层继续使用进度游标和有限重试；完整 errno 下沉仍是后续工作 |
| **M3** | 盲 RST（RFC 5961） | 伪造 RST 一键拆除任意活跃连接 → 静态站可被第三方 DoS | **已进入主线**：窗口外丢弃+challenge-ACK（`net/net_tcp.c`） | inject 用例回归（wire_lib.py 可造盲 RST） |
...
| **L8** | close 双别名（nr==3↔6↔28） | ring3 socket 仍使用 nr=28 关闭；nr=3 已恢复为 Linux read，不再误关 fd | **已收敛**（commit `289e9ce`） | 保持 socket close 使用 nr=28；ABI 文档和 sock_abi 断言已锁定现行语义 |

### 4.2 并发限制：TCP_MAX_CONNS=64 及派生天花板 ✅实测

| 资源 | 上限 | 出处 | HTTP 场景含义 |
|---|---|---|---|
| TCP 连接表 | **64** | `net/net.h:86` | 同时存在（含 LISTEN/SYN_RCVD/ESTAB/FIN 族/TIME_WAIT）≤64；可用连接数还受常驻 listener 和 VFS fd 表影响 |
| listen backlog | ≤64，bind 路径默认 **1** | `net/net.h:97-101`、`net/net_tcp.c` | ring3 bind 后必须显式 listen(n)；连接表耗尽时 SYN 直接 RST |
| 收/发缓冲 | 各 4096B | net.h:70 | 单响应 >4KB 必跨多次 send；慢读客户端反压发送缓冲→部分写或 `-EAGAIN` |
| 单段 MSS | 1460B | net.h:69 | 发送分片粒度 |
| scoreboard/OOO | 8 段/2 槽 | `net/net.h:90-96`、`net/net_tcp.c` | 乱序容忍有限，超限降级重 ACK（已并入主线加固语义） |
| TIME_WAIT | 200 ticks=2s 定长 | `net/net_tcp.c` FIN/TIME_WAIT 路径 | 快速重启压测可能触顶 RST；测试脚本须按当时 listener 数量和连接表余量计算 |
...
**压测纪律（历史设计约束）**：并发探针须按当前 listener 数量、`TCP_MAX_CONNS=64` 和 `VFS_MAX_FD=32` 计算；单响应基准 ≤4KB，大响应专项单独跑。
...
| `syscall.c`/`syscall.h` | code5/code2 链 | 当前 nr=168 poll；exec 泛化备选 A | **历史串行排队记录；当前实现见 §0** |
        ...
        └─► vfs.c 解锁 ─► Step 2 romfs 接入 ─► Step 2 验收
              ...
              └─► (备选A) syscall.c 排队: exec 泛化
                    └─► code4+code5+code10 全部归零 ─► Step 4 统筹(e1000/interrupts/net/syscall 四区)
                          └─► poll() nr=168 ─► nginx shim 前置达成（当前已落地）
```
...
## 6. 历史 NOT_TESTED 与当前验证边界

本节保留初版计划的未验证项；已经由当前实现和 r4 证据覆盖的内容改为历史限制，避免
把旧路线状态误读为当前工作区状态。

| # | 条目 | 状态 |
|---|---|---|
| N1 | 本文初版全部行为/行号结论为**静态读码**所得，零编译零运行 | **历史限制；当前 nginx 最小路径已有 r4 与独立 fresh QEMU 证据** |
| N2 | code4 工作区加固（H2/M1/M3/化身回收/scoreboard 合并）的运行时回归 | **历史记录；网络回归证据见 `docs/TEST_MATRIX.md`，不等同于 nginx 完整特性** |
...
| N6 | nginx 1.26.2 configure 对 `--with-poll_module` 的最小依赖面复核 | **已核对并实际构建；`build-nginx.sh:9-34` 显式关闭未移植模块并启用 poll，完整行为仍受当前配置裁剪** |

---

*文档结束。生成者：Cat-OS 并行任务 code3（NGINX_PORT_PLAN 实施设计）。
约束遵守声明：主体为 2026-08-25 实施设计快照；§0 和第 6 节当前状态回填于 2026-08-28。当前回填只更新文档，不修改 nginx/Cat-OS 源码。*
