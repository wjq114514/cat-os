...
int vfs_open(const char*p,uint32_t fl){for(unsigned i=0;i<sizeof(nodes)/sizeof(nodes[0]);i++){const char*a=p,*b=nodes[i].name;while(*a&&*a==*b){a++;b++;}if(!*a&&!*b)for(int fd=3;fd<VFS_MAX_FD;fd++)if(!fds[fd]){static file_t fs[VFS_MAX_FD];fs[fd]=(file_t){&nodes[i],0,fl};fds[fd]=&fs[fd];return fd;}}return -1;}
int vfs_read(int fd,void*b,uint32_t n){if(fd<0||fd>=VFS_MAX_FD||!fds[fd]||!fds[fd]->inode->ops->read)return -9;if(!user_access_ok((uintptr_t)b,n,1))return -14;return fds[fd]->inode->ops->read(fds[fd],b,n);}int vfs_write(int fd,const void*b,uint32_t n){if(!user_access_ok((uintptr_t)b,n,0))return -14;if(fd<0||fd>=VFS_MAX_FD||!fds[fd]||!fds[fd]->inode->ops->write)return -9;return fds[fd]->inode->ops->write(fds[fd],b,n);}int vfs_close(int fd){if(fd<3||fd>=VFS_MAX_FD||!fds[fd])return -9;fds[fd]=0;return 0;}
